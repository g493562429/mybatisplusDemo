package com.gn.demo.config.security;

import com.gn.demo.config.security.constant.Constant;
import com.gn.demo.config.security.filter.AdminAuthenticationProcessingFilter;
import com.gn.demo.config.security.filter.MyAuthenticationFilter;
import com.gn.demo.config.security.login.AdminAuthenticationEntryPoint;
import com.gn.demo.config.security.url.UrlAccessDecisionManager;
import com.gn.demo.config.security.url.UrlAccessDeniedHandler;
import com.gn.demo.config.security.url.UrlFilterInvocationSecurityMetadataSource;
...
//                // 自定义登录用户名和密码字段名，默认为username和password
...
//        registry.antMatchers("/home").hasRole("admin");
        // 表示只能在 服务器本地ip[127.0.0.1 或 localhost] 访问‘home’ 这个接口，其他ip地址无法访问
        registry.antMatchers("/home").hasIpAddress(Constant.LOCALHOST);

        //允许匿名的url - 放行接口集合(url白名单) - 除配置文件忽略url以外，其它所有请求都需经过认证和授权
        for (String url : myProperties.getAuth().getIgnoreUrls()) {
            registry.antMatchers(url).permitAll();
        }
//         registry.antMatchers("/**").access("hasAuthority('admin')");
        // OPTIONS(选项)：查找适用于一个特定网址资源的通讯选择。在不需执行具体的涉及数据传输的动作情况下，允许客户端来确定与资源相关的选项/要求，或是一个服务器的性能
    ...
    /**
     *  忽略拦截url或静态资源文件夹 - web.ignoring(): 会直接过滤该url - 将不会经过Spring Security 过滤器链
